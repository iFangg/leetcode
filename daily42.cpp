...
public:
    int numTeams(vector<int>& rating) {
        // index 1 2 and 3 pointers, sliding window?
        // hashmaps for all elements (in order) to the left and right of current element
        auto total = int{0};

        for (auto l = 0; l < rating.size() - 2; ++l) {
            for (auto m = l + 1; m < rating.size() - 1; m++) {
                for (auto r = m + 1; r < rating.size(); ++r) {
                    if (rating[l] < rating[m] and rating[m] < rating[r])
                        total++;
